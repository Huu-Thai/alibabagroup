...
<script type="text/javascript">
  $(document).ready(function() {
    $(".anhien").click(function() {
      ...
      var id = $(this).attr('id');
      $.post('index.php?ctr=ProductController&action=changeShowHide&x='+(new Date()).getTime(), param, function(data) {
...
<?php if($data['product'] != false): ?>
  <?php mysqli_data_seek($data['product'], 0); ?>
  <?php while ($row = mysqli_fetch_assoc($data['product']) ) : ?>
    <?php ob_start(); ?>
    <tr>
      <td class="anhien">{id}</td>
      <td style="text-align: center;text-transform: capitalize;">{nameCate}</td>
      <td>{title}</td>
      ...
      <td align="center">
        <a class="smallButton" href="index.php?ctr=ProductController&action=showEditProduct&id={id}"><img  src="images/pencil.png" title="Sửa Tin"></a>
        <a class="smallButton" href="index.php?ctr=ProductController&action=delete&id={id}" onclick="return confirm('Bạn có muốn xóa !!! ');"><img src="images/close.png" title="Xóa Tin"></a>
...
<tr>
  <td colspan="8" align="left">
   <p id=thanhphantrang>
   <?=$ProductController->pagesList($data['totalRow'], $data['pageNum'], $data['pageSize']);?>
  </p>
</td>
</tr>
<?php } ?>
